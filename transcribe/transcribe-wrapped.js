...
async function transcribe ({
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      sendToWebsocket(
        websocketConnection,
        'Whisper initializing, updates to come...'
      );
      ...
      whisperProcess.on('close', async (code) => {
        try {
          ...
          if (processFinishedSuccessfully) {
            ...
            await fs.move(
              originalUpload,
              `${containingDir}/${directorySafeFileNameWithExtension}`,
              { overwrite: true }
            );
            ...
            await fs.move(
              `${containingDir}/${uploadFolderFileName}.srt`,
              transcribedSrtFile,
              { overwrite: true }
            );

            await fs.move(
              `${containingDir}/${uploadFolderFileName}.vtt`,
              transcribedVttFile,
              { overwrite: true }
            );

            await fs.move(
              `${containingDir}/${uploadFolderFileName}.txt`,
              transcribedTxtFile,
              { overwrite: true }
            );
          ...
          `.replace(/^ +/gm, ''); // remove indentation
            ...
            if (translationStartedAndCompleted) {
              const trimmedLanguagesToTranscribe = languagesToTranscribe.filter(
                (e) => e !== language
              );
            ...
            await fs.appendFile(
              `${containingDir}/processing_data.json`,
