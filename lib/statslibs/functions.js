...
const { TranscriptionFullStats, TranscriptionInfo, TranscriptionStat } = require('./models');
...
function addLanguageStatistic(transcFullStat, transcInfo){
    ...
    const timeTranscribedForTrancription = getTimeTranscribedForTranscription(transcInfo);
    if(transcFullStat.languagesStats[transcInfo.language] == undefined){
        transcFullStat.languagesStats[transcInfo.language] = new TranscriptionStat();
        transcFullStat.languagesStats[transcInfo.language].transcriptionsCount = 1;
        transcFullStat.languagesStats[transcInfo.language].timeTranscribed = timeTranscribedForTrancription;
    }
    else {
        transcFullStat.languagesStats[transcInfo.language].transcriptionsCount++;
        transcFullStat.languagesStats[transcInfo.language].timeTranscribed += timeTranscribedForTrancription;
    }
}
