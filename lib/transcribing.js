  ...
  return function (code) {
    (async function () {
      ...
      if(!processFinishedSuccessfullyBasedOnStatusCode){
        ...
        const fullPathWithoutExtension = `${process.cwd()}/transcriptions/${numberToUse}/${numberToUse}`;

        const vttFile = `${process.cwd()}/transcriptions/${numberToUse}/${numberToUse}.vtt`;

        const { strippedText, timestampsArray } = await stripOutTextAndTimestamps(vttFile)

        // TODO: need to get actual data
        // load from processing data
