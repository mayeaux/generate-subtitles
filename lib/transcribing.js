...
const updateFileDetails = async ({websocketConnection, filename, language, model, duration}) => {
  const fileDetails = `
    Filename: ${filename}
    Language: ${language}
    Model: ${model}
  ...
  // update file details
  sendToWebsocket(websocketConnection, {message: 'fileDetails', fileDetails});
}
...
const updateDetectedLanguage = ({dataAsString, fileInfo, websocketConnection}) => {
